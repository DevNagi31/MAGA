...
const SEED_GROUPS = [
  {
    ...
    bills: [
      { id: 'b1', description: 'Hotel Night 1', amount: 240, paidBy: 'm1', splits: [{ memberId: 'm1', amount: 80 }, { memberId: 'm2', amount: 80 }, { memberId: 'm3', amount: 80 }], date: d(5), settled: false },
      { id: 'b2', description: 'Dinner at Nobu', amount: 186, paidBy: 'm2', splits: [{ memberId: 'm1', amount: 62 }, { memberId: 'm2', amount: 62 }, { memberId: 'm3', amount: 62 }], date: d(4), settled: false },
      { id: 'b3', description: 'Uber to JFK', amount: 54, paidBy: 'm3', splits: [{ memberId: 'm1', amount: 18 }, { memberId: 'm2', amount: 18 }, { memberId: 'm3', amount: 18 }], date: d(3), settled: false },
    ],
    createdAt: d(7),
  },
  {
    id: 'g2', name: 'Apartment',
    members: [
      { id: 'm4', name: 'Me', color: '#34D399', zelleId: 'demo@cashcraft.app' },
      { id: 'm5', name: 'Riley', color: '#A78BFA', zelleId: 'riley@icloud.com' },
    ],
    bills: [
      { id: 'b4', description: 'Groceries', amount: 120, paidBy: 'm4', splits: [{ memberId: 'm4', amount: 60 }, { memberId: 'm5', amount: 60 }], date: d(2), settled: false },
    ],
    createdAt: d(30),
  },
];
...
export const useExpenseStore = create(
  persist(
    (set, get) => ({
      ...
      addExpense: (expense) =>
        set((s) => ({ expenses: [{ id: `e-${Date.now()}`, createdAt: new Date().toISOString(), ...expense }, ...s.expenses] })),
      deleteExpense: (id) =>
...
export const useGroupStore = create(
  persist(
    (set, get) => ({
      groups: SEED_GROUPS,
      addGroup: (group) =>
        set((s) => ({ groups: [{ id: `g-${Date.now()}`, createdAt: new Date().toISOString(), bills: [], ...group }, ...s.groups] })),
      deleteGroup: (id) =>
      ...
      addBillToGroup: (groupId, bill) =>
        set((s) => ({
          groups: s.groups.map((g) =>
            g.id === groupId
              ? { ...g, bills: [...g.bills, { id: `b-${Date.now()}`, date: new Date().toISOString(), settled: false, ...bill }] }
              : g
          ),
        })),
      settleAllDebts: (groupId) =>
        set((s) => ({
          groups: s.groups.map((g) =>
            g.id === groupId ? { ...g, bills: g.bills.map((b) => ({ ...b, settled: true })) } : g
          ),
        })),
...
export const useOCRStore = create((set) => ({
  ...
  clearScan: () => set({ scannedItems: [], scanTotal: 0 }),
}));
